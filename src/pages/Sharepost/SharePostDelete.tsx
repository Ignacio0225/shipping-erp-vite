...
import type {Shipment} from "../../types/shipment.ts"; // Shipment 타입(게시글 데이터 구조) 임포트(실제로는 삭제에는 잘 안 쓰이지만, 타입 안정성용)
...
type Props = {
    ship_id: number; // 삭제할 게시글 id(숫자, 부모가 반드시 넘겨야 함)
    onDeleted?: () => void; // 삭제 성공 후 실행할 콜백 함수(부모가 필요하면 넘김, 생략 가능)
}


// 게시글 삭제 버튼 컴포넌트(기본 내보내기)
export default function SharePostDelete({ship_id, onDeleted}: Props) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // 삭제 성공 시 분기
            if (onDeleted) {
                onDeleted(); // 부모가 콜백 넘겼으면 콜백 함수 실행(예: 목록 새로고침, 모달 닫기 등) -><SharePostDelete ship_id={post.id} obDeleted(nav('/posts', {replace: true})) 이렇게도 가능 />
            } else {
